...
<body>
  ...
  <p class="lead">Klicke ein Türchen, um den Gutschein zu sehen — eingelöste Gutscheine werden markiert.</p>
  ...
  <div class="modal-bg" id="modalBg">
    <div class="modal" role="dialog" aria-modal="true">
      ...
      <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:12px">
        <button class="btn" id="redeemBtn">Gutschein einlösen</button>
        <button class="btn" id="closeBtn" style="background:#6b7280">Schließen</button>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    tobiList.push(types[5]); // Rückenmassage duplicate
    tobiList.push(types[1]); // Nackenmassage duplicate

    // sortierung: wir wollen eine regelmäßige Rotation Tobi - Nicole - Leni repeated 7x (21)
    // und dann 3 zusätzliche Tobi einfügen verteilt (insert at indices 6,13,20) to keep spacing
    let nameSeq = [];
    for(let i=0;i<7;i++){
    ...
    function createDoor(door, index){
      ...
      el.addEventListener('click', ()=>{
        ...
        setTimeout(()=>{
          ...
          // show modal
          modalTitle.textContent = `${door.name} — Tür ${index+1}`;
